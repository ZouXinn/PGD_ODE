...
if __name__=="__main__":
    ...
    parser.add_argument("--step_size", required=False, type=float, default=0.1)
    parser.add_argument("--step_num", required=False, type=int, default=5000)
    parser.add_argument("--seed", required=False, type=int, default=2)
    
    parser.add_argument("--x0", required=False, type=float, nargs='+', choices=[None, [1, 0, 0]], default= [1, 0, 0])
    ...
    if args.x0 is not None:
        x0 = np.array(args.x0)
        x_star = np.array([1-args.eps, 0, 0])
        delta = args.eps * np.array([math.cos(math.radians(150)), math.sin(math.radians(150)), math.sin(math.radians(135))])
    else:
        x0 = np.array([0, 0, 0])
        x_star = np.array([np.sqrt(2)/2 * args.eps, np.sqrt(2)/2 * args.eps, np.sqrt(2)/2 * args.eps])
        delta = np.array([args.eps, 0, 0])
    ...
    if args.x0 is None:
        ...
        path_path = os.path.join(args.save_path, "_path_{}_{}".format(args.x0, args.step_size))
