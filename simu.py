...
if __name__=="__main__":
    ...
    # path related
    parser.add_argument("--save_path", required=False, type=str, default="./run_results/simu")
    
    # tuning parameters
    parser.add_argument("--eps", required=False, type=float, default=0.1)
    parser.add_argument("--step_size", required=False, type=float, default=0.00002)
    ...
    if args.x0 is not None:
        ...
        delta = np.array([math.cos(math.radians(150)), math.sin(math.radians(150))])
    else:
        x0 = np.array([0.05, 0])
        x_star = np.array([0, 0])
        # delta = np.array([1.2/20, 1.3/20])
    ...
    if args.x0 is None:
        ...
        norm_path = args.save_path + "_norm_{}".format(args.x0)
        path_path = args.save_path + "_path_{}".format(args.x0)
